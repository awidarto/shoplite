...
(function ($) {
    ...
    var _methods = {
        init: function (options) {
            ...
            if (target.length > 0) {
                ...
                $(tbCell).append($('<button/>').append('<b class="icon-add"></b>').attr({ type: 'button', title: settings.textResources.append }).button({ icons: { primary: 'ui-icon-plusthick' }, text: false }).click(function () { insertRow(tbWhole, 1, null, null); }));
                $(tbCell).append($('<button/>').append('<b class="icon-remove-2"></b>').attr({ type: 'button', title: settings.textResources.removeLast }).button({ icons: { primary: 'ui-icon-closethick' }, text: false }).click(function () { removeRow(tbWhole, null, this.value, false); }));
                // Save options
    ...
    function insertRow(tbWhole, numOfRow, rowIndex, callerUniqueIndex) {
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            $(tbCell).append($('<button/>').append('<b class="icon-plus-alt"></b>').addClass('insert').val(uniqueIndex).attr({ id: settings.idPrefix + '_Insert_' + uniqueIndex, type: 'button', title: settings.textResources.insert, tabindex: -1 }).button({ icons: { primary: 'ui-icon-arrowreturnthick-1-w' }, text: false }).click(function () {
                $(tbWhole).appendGrid('insertRow', 1, null, this.value);
            }));
            $(tbCell).append($('<button/>').append('<b class="icon-minus-alt"></b>').addClass('delete').val(uniqueIndex).attr({ id: settings.idPrefix + '_Delete_' + uniqueIndex, type: 'button', title: settings.textResources.remove, tabindex: -1 }).button({ icons: { primary: 'ui-icon-trash' }, text: false }).click(function () {
                removeRow(tbWhole, null, this.value, false);
            }));
            $(tbCell).append($('<button/>').append('<b class="icon-arrow-up-alt1"></b>').addClass('moveUp').val(uniqueIndex).attr({ id: settings.idPrefix + '_MoveUp_' + uniqueIndex, type: 'button', title: settings.textResources.moveUp, tabindex: -1 }).button({ icons: { primary: 'ui-icon-arrowthick-1-n' }, text: false }).click(function () {
                $(tbWhole).appendGrid('moveUpRow', null, this.value);
            }));
            $(tbCell).append($('<button/>').append('<b class="icon-arrow-down-alt1"></b>').addClass('moveDown').val(uniqueIndex).attr({ id: settings.idPrefix + '_MoveDown_' + uniqueIndex, type: 'button', title: settings.textResources.moveDown, tabindex: -1 }).button({ icons: { primary: 'ui-icon-arrowthick-1-s' }, text: false }).click(function () {
